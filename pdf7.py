...
if not filename:
    raise RuntimeError("INPUT_EXCEL_PATH is required for pdf7.py")
...
# Batterie_vergleich: VIN found AND Batterie column filled → OK (weiterverrechnet)
merged_df["Batterie_vergleich"] = merged_df.apply(
    lambda row: (
        "OK" if row["VIN_vergleich"] == "OK" and not is_empty(row.get("Batterie"))
        else "NOK"
    ),
    axis=1
)
...
def bemerkungen_logic(row):
    ...
    elif row["Batterie_vergleich"] == "NOK":
        parts.append("WFP Batterie nicht aktiviert, bitte prüfen")
    return " | ".join(parts)
...
for _, row in merged_df.iterrows():
    if row["Bemerkungen"] and str(row["Bemerkungen"]).strip() != "":
        compare_results.append({
            ...
            "WFP_Batterie_Preis": row.get("Batterie"),
